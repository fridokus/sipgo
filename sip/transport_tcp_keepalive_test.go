...
// TestReadConnectionKeepAliveMidMessage covers the defect directly. A TCP read
// that happens to contain nothing but the LF of a header line's CRLF used to be
// swallowed as an RFC 5626 keep-alive, which left the parser holding a bare CR.
// No later byte can complete it, so the *next* message -- entirely well formed
// -- fails to frame with "line has no CRLF", and so does every message after it.
//
// Every CRLF in the message is tried, because the read boundary is TCP's choice
// and not ours. Removing the !midMessage guard turns this red at all 22
// boundaries with "delivered no messages at all".
...
//
// Swapping the guard for par.Buffer().Len() == 0 turns this red as
//
